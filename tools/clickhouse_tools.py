...
from utils.logging import ToolExecutionLogger, get_logger
...
class ClickHouseToolExecutor:
    ...
    async def execute_clickhouse_query(self, query: str) -> str:
        ...
        try:
            ...
            tool_logger.log_tool_success(
                "execute_clickhouse_query", f"Returned {len(df)} rows"
            )
            return output
    ...
    async def list_tables(self) -> str:
        ...
        try:
            ...
            if len(df) > 0:
                ...
                # Return JSON data
                result_data = {"tables": table_names, "count": len(table_names)}
                output = json.dumps(result_data, indent=2)
            else:
                result_data = {"tables": [], "count": 0}
                output = json.dumps(result_data, indent=2)

            tool_logger.log_tool_success(
                "list_tables", f"Found {len(table_names) if len(df) > 0 else 0} tables"
            )
            return output
    ...
    async def get_table_schema(self, table_name: str) -> str:
        ...
        try:
            ...
            tool_logger.log_tool_success(
                "get_table_schema",
                f"Retrieved schema for {table_name} with {len(columns)} columns",
            )
            return output
    ...
    async def search_table(
        self, table_name: str, limit: int = 100, where_clause: str = None
    ) -> str:
        """Search and preview data in a table"""

        tool_logger.log_tool_start(
            "search_table",
            {"table_name": table_name, "limit": limit, "where_clause": where_clause},
        )
        ...
        try:
            ...
            if len(df) > 0:
                ...
                )  # default=str handles dates/timestamps
            else:
                ui.console.print("[dim yellow]No results found[/dim yellow]")
                result_data = {
                    "table_name": table_name,
            ...
            tool_logger.log_tool_success(
                "search_table", f"Searched table {table_name}, found {len(df)} rows"
            )
            return output
    ...
    async def export_data_to_csv(
        self, query: str, filename: str = None, analysis_limit: int = 50
    ) -> str:
        ...
        try:
            ...
            ui.console.print()
            ui.console.print(
                f"[dim bright_green]📁[/dim bright_green] [bright_white]Data Export Complete![/bright_white]"
            )
            ...
            ui.console.print(
                f"[yellow]💡 Open the CSV file for complete data analysis[/yellow]"
            )
            ui.console.print()

            tool_logger.log_tool_success(
                "export_data_to_csv", f"Exported {len(df)} rows to {filepath}"
            )
    ...
    async def export_query_results(
        self, query: str, filename: str = None, format: str = "csv", limit: int = None
    ) -> str:
        """Export large query results to file for datasets too big to display"""

        tool_logger.log_tool_start(
            "export_query_results",
            {"query": query[:100], "format": format, "limit": limit},
        )
        ...
        try:
            ...
            ui.console.print()
            ui.console.print(
                f"[dim bright_green]📁[/dim bright_green] [bright_white]Export completed![/bright_white]"
            )
            ui.console.print(f"[dim]File: {filepath}[/dim]")
            ui.console.print(
                f"[dim]Size: {file_size_mb:.2f} MB ({len(df):,} rows)[/dim]"
            )
            ui.console.print()

            tool_logger.log_tool_success(
                "export_query_results", f"Exported {len(df)} rows to {filepath}"
            )
