...
class DataLoader:
    ...
    async def load_from_csv(
        ...
        try:
            ...
            if create_table:
                ...
                console.print(
                    f"[green]✓ Table {table_name} created successfully[/green]"
                )
            ...
            with Progress() as progress:
                task = progress.add_task(
                    f"Loading data into {table_name}", total=total_rows
                )
    ...
    async def load_from_json(
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                raise ValueError(
                    "JSON file must contain a list of objects or a single object"
                )
    ...
    def _generate_create_table_sql(self, df: pd.DataFrame, table_name: str) -> str:
        ...
        # Bind the separator to a name so the f-string expression
        # part stays free of backslashes — that's a SyntaxError on
        # Python 3.8 - 3.11 (only legal from 3.12 onward) and clickr
        # supports the older versions per pyproject.toml's matrix.
        sep = ",\n    "
        create_sql = f"""
CREATE TABLE IF NOT EXISTS {table_name} (
    {sep.join(column_definitions)}
) ENGINE = MergeTree()
ORDER BY tuple()
"""
...
class DataExporter:
    ...
    async def export_to_file(
        self, query: str, output_path: str, format: str = "csv"
    ) -> str:
        """Export query results to file"""

        tool_logger.log_tool_start(
            "export_to_file",
            {"query": query[:100], "output_path": output_path, "format": format},
        )
