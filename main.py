...
from ui.onboarding import OnboardingFlow, needs_onboarding
...
def chat(
    ...
    host: Optional[str] = typer.Option(None, "--host", "-h", help="ClickHouse host"),
    port: Optional[int] = typer.Option(None, "--port", "-p", help="ClickHouse port"),
    username: Optional[str] = typer.Option(
        None, "--username", "-u", help="ClickHouse username"
    ),
    password: Optional[str] = typer.Option(
        None, "--password", help="ClickHouse password"
    ),
    database: Optional[str] = typer.Option(
        None, "--database", "-d", help="ClickHouse database"
    ),
    ...
    ui.show_connection_status(
        host=config.host, port=config.port, database=config.database, connected=True
    )
...
def load_data(
    ...
    create_table: bool = typer.Option(
        True, "--create-table", help="Create table if it doesn't exist"
    ),
    batch_size: int = typer.Option(
        10000, "--batch-size", help="Batch size for data loading"
    ),
):
    ...
    try:
        agent = ClickHouseAgent(config)
        asyncio.run(
            agent.load_data_from_file(file_path, table, create_table, batch_size)
        )
    except Exception as e:
        console.print(f"[red]❌ Error: {e}[/red]")
        sys.exit(1)
...
def clear():
    ...
    if Confirm.ask(
        "[bold red]Are you sure you want to clear all configuration?[/bold red]"
    ):
        for file in existing_files:
            try:
                file.unlink()
        ...
        console.print()
        console.print(
            "[green]🎉 Configuration cleared! Run 'proto' to start fresh onboarding.[/green]"
        )
    else:
...
def refresh_template():
    ...
    try:
        ...
        provider.chat_template_path = os.path.join(
            provider.cache_dir,
            f"{provider.model_name.replace('/', '_')}_{provider.chat_template_file}",
        )
        ...
        if success:
            ...
            console.print(
                f"[dim]Template location: {provider.chat_template_path}[/dim]"
            )
            ...
            if os.path.exists(provider.chat_template_path):
                with open(provider.chat_template_path, "r") as f:
                    content = f.read()
                    console.print(
                        f"[dim]Template size: {len(content)} characters[/dim]"
                    )
            ...
            console.print(
                "[yellow]The agent will use the default template if available[/yellow]"
            )
            sys.exit(1)
