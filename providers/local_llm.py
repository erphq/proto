...
class LocalLLMProvider:
    ...
    def __init__(
        ...
        model: str = "vishprometa/clickhouse-qwen3-1.7b-gguf",
    ):
        ...
        self.model_url = (
            f"https://huggingface.co/{self.model_name}/resolve/main/{self.model_file}"
        )
        self.cache_dir = os.path.expanduser("~/.cache/llama.cpp")
        self.model_path = os.path.join(
            self.cache_dir, f"{self.model_name.replace('/', '_')}_{self.model_file}"
        )
        ...
        self.chat_template_path = os.path.join(
            self.cache_dir,
            f"{self.model_name.replace('/', '_')}_{self.chat_template_file}",
        )
    ...
    def _auto_setup(self):
        ...
        try:
            ...
            model_exists = (
                ...
                and os.path.getsize(self.model_path) > 3_000_000_000
            )
            ...
            logger.info(
                f"Template status: exists={template_exists}, path={self.chat_template_path}"
            )
            ...
            if not model_exists:
                ...
                if not self._download_chat_template():
                    logger.warning(
                        "Chat template download failed, will try to start server without custom template"
                    )
            ...
            # If model exists but template doesn't (existing users), download just the template
                ...
                logger.info(
                    "Model exists but template missing, downloading template..."
                )
                self._announce(
                    "📥 Downloading missing chat template for existing model..."
                )
                if not self._download_chat_template():
                    logger.warning(
                        "Chat template download failed, will try to start server without custom template"
            ...
            if self._is_server_running():
                self._announce(
                    f"✅ ClickHouse AI Agent ready on {self.host}:{self.port}"
                )
                return
        ...
        try:
            ...
            if os.path.exists(self.chat_template_path):
                ...
                if file_age < 86400:  # 24 hours in seconds
                    if not quiet:
                        logger.info(
                            f"Chat template already exists and is recent: {self.chat_template_path}"
                        )
                    return True
                else:
                    if not quiet:
            ...
            logger.info(
                f"Chat template saved: {self.chat_template_path} ({file_size} bytes)"
            )

            if not quiet:
                self._announce(
                    f"✅ Chat template downloaded: {self.chat_template_path}"
                )
            return True

        except requests.RequestException as e:
            logger.error(
                f"Failed to download chat template from {self.chat_template_url}: {e}"
            )
            return False
        except Exception as e:
        ...
        try:
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"🔄 Downloading {self.display_name}...", total=total_size
                )

                # Download with requests and stream
                response = requests.get(
                    self.model_url, stream=True, allow_redirects=True
                )
                response.raise_for_status()
                ...
                if downloaded_size > 3_000_000_000:  # > 3GB
                    ...
                    self._announce(
                        f"✅ Model downloaded successfully: {self.model_path}"
                    )
                    ...
                    if self._download_chat_template(quiet=True):
                        progress.update(
                            task, description="✅ Model and template ready!"
                        )
                    else:
                        progress.update(
                            task,
                            description="✅ Model ready (template download failed)",
                        )
            ...
            with Progress(
                    ...
                    if current_time - last_update_time > 2.0:  # Update every 2 seconds
                        if downloaded_size < total_size:
                            ...
                            if elapsed > 0 and downloaded_size > 0:
                                ...
                                progress.update(
                                    task,
                                    description=f"🔄 Downloading {self.display_name}... ({downloaded_size}/{total_size} bytes, {speed/1024/1024:.1f} MB/s)",
                                )
                ...
                if process.returncode == 0 and os.path.exists(self.model_path):
                    ...
                    if file_size > 3_000_000_000:  # > 3GB
                        ...
                        # Download chat template immediately after successful model download
                        progress.update(
                            task, description="📥 Downloading chat template..."
                        )
                        if self._download_chat_template(quiet=True):
                            progress.update(
                                task, description="✅ Model and template ready!"
    ...
    def _build_llama_cpp_from_source(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            subprocess.run(
                ["sudo", "cp", llama_server_src, llama_server_dst], check=True
            )
    ...
    def _start_server(self):
        ...
        try:
            self._announce(
                f"🚀 Starting ClickHouse AI Agent on {self.host}:{self.port}"
            )
            ...
            cmd = [
                "llama-server",
                "-m",
                self.model_path,  # Use local model file instead of Hugging Face URL
                "--jinja",
            ]
            ...
            if self.chat_template_path and os.path.exists(self.chat_template_path):
                ...
                logger.warning(
                    "❌ No custom chat template found, using llama-server default"
                )
                # Removed the announcement message as requested
            ...
            logger.info(
                f"Started ClickHouse AI Agent with PID {self.server_process.pid}"
            )
            ...
            if self.server_process.poll() is not None:
                ...
                raise RuntimeError(
                    f"llama-server failed to start. Exit code: {self.server_process.returncode}"
                )
            else:
                logger.info(
                    "llama-server process is running, proceeding with startup..."
                )
    ...
    def _kill_existing_server(self):
        ...
        try:
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    if "llama-server" in proc.info[
                        "name"
                    ] or "llama-server" in " ".join(proc.info["cmdline"] or []):
                        # Check if it's using our port
                        cmdline = " ".join(proc.info["cmdline"] or [])
                        if (
                            f"--port {self.port}" in cmdline
                            or f":{self.port}" in cmdline
                        ):
                            logger.info(
                                f"Stopping existing ClickHouse AI Agent process {proc.info['pid']}"
                            )
                            proc.terminate()
                            try:
                                proc.wait(timeout=5)
    ...
    def _wait_for_server(self, timeout: int = 60):
        ...
        with Progress(
            ...
            progress.update(
                task, description="❌ Timeout waiting for ClickHouse AI Agent"
            )
            raise TimeoutError(
                f"ClickHouse AI Agent did not start within {timeout} seconds"
            )
