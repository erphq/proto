...
class MinimalInterface:
    ...
    def show_connection_status(
        self, host: str, port: int, database: str, connected: bool = True
    ):
    ...
    def show_tool_execution(
        self, tool_name: str, description: str = "", arguments: dict = None
    ):
        ...
        self.console.print(
            f"[dim bright_yellow]🔧[/dim bright_yellow] [bright_white]Executing tool: {tool_name}[/bright_white]"
        )
        if arguments:
            ...
            for key, value in arguments.items():
                ...
                self.console.print(
                    f"[dim]{key}>[/dim] [dim bright_white]{display_value}[/dim bright_white]"
                )
    ...
    def show_success(self, message: str):
        ...
        self.console.print(
            f"[dim bright_green]✓[/dim bright_green] [bright_white]{message}[/bright_white]"
        )

    def show_error(self, message: str):
        """Minimal error message with spacing"""
        self.console.print(
            f"[dim red]✗[/dim red] [bright_white]{message}[/bright_white]"
        )

    def show_warning(self, message: str):
        """Minimal warning message with spacing"""
        self.console.print(
            f"[dim yellow]⚠[/dim yellow] [bright_white]{message}[/bright_white]"
        )
    ...
    def show_agent_response(self, message: str):
        ...
        self.console.print(
            f"[dim bright_blue]●[/dim bright_blue] [bright_white]{message}[/bright_white]"
        )
        self.console.print()
    ...
    def show_agent_response_markdown(self, message: str):
        ...
        if any(
            marker in message for marker in ["##", "**", "|", "```", "- ", "1. ", "*"]
        ):
            # Add a subtle bullet point and render as markdown
            self.console.print(
                f"[dim bright_blue]●[/dim bright_blue] [dim]Response:[/dim]"
            )
            self.console.print()
    ...
    def show_reasoning(self, reasoning_text: str):
        ...
        self.console.print(
            f"[dim bright_yellow]💭[/dim bright_yellow] [dim italic]Thinking:[/dim italic]"
        )
        ...
        for line in lines:
            if line.strip():  # Only show non-empty lines
                ...
                self.console.print(
                    f"[dim]│[/dim] [dim italic]{line.strip()}[/dim italic]"
                )
    ...
    def show_query_execution(self, query: str):
        ...
        self.console.print(
            f"[dim bright_magenta]SQL>[/dim bright_magenta] [dim bright_white]{query}[/dim bright_white]"
        )
        self.console.print()
    ...
    def show_data_table(
        ...
        if total_rows > 1000:
            self.console.print()
            self.console.print(
                f"[dim bright_yellow]📊[/dim bright_yellow] [bright_white]Large dataset: {total_rows:,} total rows[/bright_white]"
            )
            self.console.print(
                f"[dim]Showing first {min(max_rows, len(data))} rows. Use LIMIT for specific ranges.[/dim]"
            )
        ...
        if len(columns) > 8:
            self.console.print()
            self.console.print(
                f"[dim bright_blue]📋[/dim bright_blue] [bright_white]Wide table: {len(columns)} columns[/bright_white]"
            )
            self.console.print(
                f"[dim]Showing first 8 columns. Use SELECT col1, col2... for specific columns.[/dim]"
            )
            display_columns = columns[:8]

        # Create minimal table with smart title
        rows_text = (
            f"{min(len(data), max_rows):,} of {total_rows:,}"
        ...
        if len(data) > displayed_rows or total_rows > len(data):
            ...
            if remaining > 0:
                ...
                table.add_row(
                    *[
                        f"[dim]{pagination_text}[/dim]" if i == 0 else "[dim]...[/dim]"
        ...
        if total_rows > 100:
            self.console.print()
            self.console.print(
                f"[dim bright_cyan]💡[/dim bright_cyan] [dim]Tips: Use LIMIT, WHERE clauses, or ask to export results[/dim]"
            )
        elif len(columns) > 8:
            self.console.print()
            self.console.print(
                f"[dim bright_cyan]💡[/dim bright_cyan] [dim]Tip: Use SELECT specific_columns FROM table for focused results[/dim]"
            )

        self.console.print()

    def show_query_result(
        self, query: str, result_data: List[Dict], execution_time: float = None
    ):
    ...
    def show_goodbye(self):
        ...
        self.console.print()
        self.console.print(
            "[dim bright_cyan]✨[/dim bright_cyan] [bright_white]Thanks for using Proto![/bright_white]"
        )
        self.console.print()
    ...
    def show_statistics(self, stats: Dict[str, Any]):
        ...
        for key, value in stats.items():
            ...
            self.console.print(
                f"[dim]{display_key}:[/dim] [bright_white]{value}[/bright_white]"
            )
