...
class SettingsManager:
    ...
    def show_current_settings(self):
        ...
        if provider == "local":
            ai_table.add_row(
                "🌐 Base URL:", config.get("local_llm_base_url", "Not set")
            )
            ...
            ai_table.add_row(
                "🔑 API Key:", "••••••••" if config.get("openai_api_key") else "Not set"
            )
        ...
        ch_table.add_row(
            "🔒 Password:",
            "••••••••" if config.get("clickhouse_password") else "Not set",
        )
        ch_table.add_row("🗄️  Database:", config.get("clickhouse_database", "Not set"))
        ch_table.add_row(
            "🔐 Secure:", "Yes" if config.get("clickhouse_secure") else "No"
        )
        ...
        agent_table.add_row(
            "🌡️  Temperature:", str(config.get("temperature", "Not set"))
        )
        agent_table.add_row("📝 Max Tokens:", str(config.get("max_tokens", "Not set")))
        agent_table.add_row(
            "🔧 Max Tool Calls:", str(config.get("max_tool_calls", "Not set"))
        )
    ...
    def edit_ai_provider(self):
        ...
        console.print()
        ui.show_info(
            "ClickHouse AI Agent uses your own local model. No provider changes needed."
        )
        console.print()
    ...
    def edit_clickhouse(self):
        ...
        console.print()
        if Confirm.ask(
            "[bold bright_cyan]Update ClickHouse settings?[/bold bright_cyan]"
        ):
    ...
    def reset_config(self):
        ...
        console.print()
        if Confirm.ask(
            "[bold red]⚠️  Reset all settings and run setup again?[/bold red]"
        ):
            if self.config_file.exists():
