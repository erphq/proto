...
from rich.box import DOUBLE, MINIMAL, ROUNDED, SIMPLE
...
class BeautifulInterface:
    ...
    def show_welcome_screen(self):
        ...
        subtitle = Text(
            "Intelligent Database Analysis & Operations", style="italic bright_white"
        )
    ...
    def show_thinking_animation(self, message: str = "Thinking..."):
        ...
        return Live(
            Align.center(
                Text.from_markup(
                    f"[dim bright_cyan]⠋[/dim bright_cyan] [bright_white]{message}[/bright_white]"
                )
            ),
    ...
    def show_tool_execution(self, tool_name: str, description: str = ""):
        ...
        return Live(
            Align.center(
                Text.from_markup(
                    f"[dim bright_yellow]⠸[/dim bright_yellow] [bright_white]{clean_name}[/bright_white]"
                )
            ),
            refresh_per_second=8,
        )
    ...
    def show_data_table(
        self, data: List[Dict], title: str = "Results", max_rows: int = 10
    ):
        ...
        if data:
            for key in data[0].keys():
                table.add_column(
                    str(key).replace("_", " ").title(), style="bright_white"
                )
    ...
    def show_user_input_prompt(self):
        ...
        prompt_text = Text.assemble(
            ("💬 ", "bright_cyan"), ("You: ", "bold bright_cyan")
        )
        ...
        goodbye_panel = Panel(
            Align.center(
                Text.assemble(
                    ("👋 ", "bold bright_yellow"),
                    (
                        "Thank you for using Proto ClickHouse AI Agent!",
                        "bold bright_white",
                    ),
                    ("\n\n", ""),
